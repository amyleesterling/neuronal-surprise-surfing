...
from codex.service import surprise_scoring
...
def index(path):
    if request.args.get("filter_string"):
        ...
        card_data = [
            {
                "header": "Surprise",
                "body": "Start with a neuron that breaks the pattern — anomalies as doors, not search results",
...
def _attach_visuals(card, data_version):
    d = card.as_template_dict()
    d["thumbnail_png"] = url_for_skeleton(card.root_id, file_type="png")
    d["thumbnail_gif"] = url_for_skeleton(card.root_id, file_type="gif")
    d["ngl_url"] = nglui.url_for_cells([card.root_id], data_version=data_version)
    d["cell_details_url"] = url_for("app.cell_details", root_id=card.root_id)
    return d


def _parse_path_arg(raw):
    if not raw:
        return []
    out = []
    for tok in raw.split(","):
        ...
        try:
            ...
            continue
    # de-dupe while preserving order (a journey may revisit, but the URL keeps last)
    seen = set()
    deduped = []
    for rid in out:
        if rid not in seen:
...
@base.route("/surprise", methods=["GET"])
def surprise():
    """Surprise surfing — a journey through the connectome where each step is
    an anomaly. The page has two modes selected by ?path=:

      - empty path  -> seed feed (entry points; "where will you start?")
      - path=id1,id2,id3 -> journey mode: persistent 3D canvas showing the
        entire walked path, breadcrumb of past steps, and a sidebar of
        contextually-related surprises emerging from the current cell.

    The user arrives without a question. The question forms. The path keeps
    forming questions.
    """
    data_version = request.args.get("data_version", "") or DEFAULT_DATA_SNAPSHOT_VERSION
    path_ids = _parse_path_arg(request.args.get("path", ""))
    logger.info(f"Loading Surprise (path={path_ids}) for version={data_version}")

    neuron_db = NeuronDataFactory.instance().get(version=data_version)
    feed = surprise_scoring.get_feed(neuron_db, data_version)

    # ----- Seed feed (no path yet) -----
    if not path_ids:
        return render_template(
            "surprise.html",
            mode="seed",
            hero=_attach_visuals(feed.hero, data_version) if feed.hero else None,
            degree_outliers=[_attach_visuals(c, data_version) for c in feed.degree_outliers],
            cross_region_bridges=[_attach_visuals(c, data_version) for c in feed.cross_region_bridges],
    ...
    trail_ids = path_ids[:-1]

    nd_current = neuron_db.neuron_data.get(current_id) or {}
    current_card_dict = {
        "root_id": current_id,
        "name": nd_current.get("name") or str(current_id),
        "cell_type": (nd_current.get("cell_type") or [""])[0]
        if isinstance(nd_current.get("cell_type"), list)
        else (nd_current.get("cell_type") or ""),
        "super_class": nd_current.get("super_class") or "",
        "nt_type": nd_current.get("nt_type") or "",
        "side": nd_current.get("side") or "",
        "input_cells": int(nd_current.get("input_cells") or 0),
        "output_cells": int(nd_current.get("output_cells") or 0),
        "input_neuropils": list(nd_current.get("input_neuropils") or []),
        "output_neuropils": list(nd_current.get("output_neuropils") or []),
        "thumbnail_gif": url_for_skeleton(current_id, file_type="gif"),
        "cell_details_url": url_for("app.cell_details", root_id=current_id),
    }

    # Trail cards (small, for breadcrumb).
    trail_cards = []
    for rid in trail_ids:
        nd = neuron_db.neuron_data.get(rid) or {}
        trail_cards.append({
            "root_id": rid,
            "name": nd.get("name") or str(rid),
            "thumbnail_png": url_for_skeleton(rid, file_type="png"),
            "step_url": url_for("base.surprise", path=",".join(str(x) for x in path_ids[:trail_ids.index(rid) + 1])),
        })

    # Related surprises — generates the next step.
    related_cards = surprise_scoring.compute_related(
        neuron_db, current_id, data_version, n=12
    )
    related = []
    for c in related_cards:
        d = _attach_visuals(c, data_version)
        # Each related card extends the current path.
        next_path = path_ids + [c.root_id]
        d["next_url"] = url_for(
            "base.surprise",
            path=",".join(str(x) for x in next_path),
            data_version=data_version if data_version != DEFAULT_DATA_SNAPSHOT_VERSION else None,
        )
        related.append(d)

    journey_ngl_url = nglui.url_for_journey(
        current_id, trail_ids, data_version, neuron_db=neuron_db
    )
    ...
    return render_template(
        ...
        num_synapses=display(neuron_db.num_synapses()),
        path=path_ids,
        path_str=",".join(str(x) for x in path_ids),
        current=current_card_dict,
        trail=trail_cards,
        related=related,
        journey_ngl_url=journey_ngl_url,
    )
