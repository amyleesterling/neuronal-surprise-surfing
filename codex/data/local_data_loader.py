...
UTC = timezone.utc  # back-compat shim: datetime.UTC is 3.11+; this works on 3.9+
    ...
    try:
        ...
        with gzip.open(pf, "rb") as handle:
            ...
            if version == DEFAULT_DATA_SNAPSHOT_VERSION:
                # Strict check disabled: the published GCS pickle is a superset
                # of the public source schema (extra fields like resolved_type,
                # gene, cell_type_source). Refusing to load on superset diffs
                # makes a working pickle look broken. We only fail on missing
                # required fields.
                expected_keys = set(NEURON_DATA_ATTRIBUTE_TYPES.keys())
                sample = next(iter(db.neuron_data.values()), None)
                if sample is not None:
                    missing = expected_keys - set(sample.keys())
                    if missing:
                        logger.error(
                            f"Pickle for {version=} is missing required fields: {sorted(missing)}. "
                            f"Delete static/data/{version}/ and re-download."
                        )
                        exit(1)
                    extra = set(sample.keys()) - expected_keys
                    if extra:
                        logger.info(
                            f"Pickle has {len(extra)} extra field(s) beyond the public schema "
