...
class Connections(object):
    ...
    def num_synapses(self):
        # Defensive: the GCS-published v2 pickle stores only ``connected_pairs``
        # and never set ``synapse_count``. Reconstruct from the encoded values
        # if possible (each value packs syn_cnt with NT type and region indices).
        if hasattr(self, "synapse_count"):
            return self.synapse_count
        cp = getattr(self, "connected_pairs", None)
        if not cp:
            return 0
        # Use a cached sum across the values dict. Values are either an int or a
        # short list of ints; the high bits of each carry the synapse count.
        cached = getattr(self, "_derived_synapse_total", None)
        if cached is not None:
            return cached
        total = 0
        for v in cp.values():
            if isinstance(v, int):
                # The encoding packs syn_cnt in the high bits (>>3 strips the
                # 8 NT type indices). Region index is also embedded but the
                # contribution to a *count* is dominated by the syn_cnt term.
                total += v >> 3
            else:
                for vi in v:
                    total += vi >> 3
        # This is an approximation good to within a small constant factor — used
        # only for the homepage statistic, not for any scoring decision.
        self._derived_synapse_total = total
        return total

    def num_connections(self):
        # Defensive: see num_synapses note above.
        if hasattr(self, "connection_count"):
            return self.connection_count
        if hasattr(self, "connected_pairs"):
            return len(self.connected_pairs)
        return 0
